...
<header>
    <a href="{{ route('books.index') }}">
        <h1 class="mb-10 text-5xl">B<span style="color: red">oo</span>k</h1>
    </a>
</header>

<form action="{{ route('books.index') }}" type="GET" class="flex pb-10 space-x-2">
    <input type="hidden" name="filter" value="{{ request('filter') }}"/>
    <input type="text" name="title" placeholder="Search by title" value="{{ request('title') }}" class="input h-10"/>
    <button type="submit" class="btn h-10">Search</button>
    <a type="submit" class="btn" href="{{ route('books.index') }}">Clear</a>
</form>

<div class="filter-container mb-10 flex">
